...
- **URL**: https://demo-swf-app-chris--adxmnzb.nicepebble-f502e190.eastus2.azurecontainerapps.io
- **Platform**: Azure Container Apps (eastus2)
- **Database**: PostgreSQL Flexible Server (Burstable B1ms, v16), DB name `postgres`, admin user `psqladmin`
- **Terraform state**: Local, stored in `terraform-azure/terraform.tfstate`
- **Container registry**: ACR at `demoswfappchrisacr.azurecr.io`
...
### Redeploying to Azure After Code Changes
```bash
# 1. Build image for AMD64
docker build --platform linux/amd64 -t demoswfappchrisacr.azurecr.io/demo-swf-app-chris:latest .

# 2. Push to ACR
az acr login --name demoswfappchrisacr
docker push demoswfappchrisacr.azurecr.io/demo-swf-app-chris:latest

# 3. Update container app
az containerapp update --name demo-swf-app-chris --resource-group demo-swf-app-chris-rg --image demoswfappchrisacr.azurecr.io/demo-swf-app-chris:latest
```
...
- **Region**: Must use eastus2 — eastus has PostgreSQL Flexible Server provisioning restrictions (`LocationIsOfferRestricted`)
...
- The `postgres` database is auto-created by PostgreSQL Flexible Server — if rebuilding from scratch, use `terraform import` for `azurerm_postgresql_flexible_server_database.main`
- Container Apps Environment has `lifecycle { ignore_changes = [infrastructure_resource_group_name] }` — without this, Azure's computed managed resource group name triggers unnecessary destroy/recreate cycles
- On initial deploy, push the Docker image to ACR before creating the Container App, or the Container App will enter a `Failed` provisioning state (image not found). If this happens, delete the failed Container App (`az containerapp delete`) and re-apply
